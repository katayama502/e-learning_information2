    ...
    persist(
        (set, get) => ({
            ...
            logout: async () => {
                ...
                // 1. Call Client Supabase SignOut FIRST
                const supabase = createClient();
                try {
                    await supabase.auth.signOut();
                } catch (e) {
                    console.warn('Client Supabase SignOut warning:', e);
                }
            ...
            sendMessage: async (threadId, senderId, text, attachment, replyToId, fileToUpload) => {
                ...
                console.log('Sending message:', { threadId, senderId, text, attachment: attachment ? 'present' : 'none', file: fileToUpload ? 'present' : 'none', replyToId });
