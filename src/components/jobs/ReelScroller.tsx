...
import { Heart, MessageCircle, Share2, ChevronUp, ChevronDown, Zap } from 'lucide-react';
...
export default function ReelScroller() {
    ...
    // 体験JOBと注目の求人を表示（appStoreから取得）
    const displayJobs = jobs.filter(j => j.type === 'quest' || j.category === '中途' || j.category === '新卒');

    // データが無い場合の表示
    if (displayJobs.length === 0) {
        return (
            <div className="relative w-full max-w-md h-[75vh] md:h-[80vh] mx-auto bg-black rounded-[3rem] overflow-hidden shadow-2xl border-4 border-zinc-900 mt-4 md:mt-10 flex items-center justify-center">
                <p className="text-white/50 font-bold text-sm">表示するリールがありません</p>
            </div>
        );
    }
        ...
        <div className="relative w-full max-w-md h-[75vh] md:h-[80vh] mx-auto bg-black rounded-[3rem] overflow-hidden shadow-2xl border-4 border-zinc-900 mt-4 md:mt-10">
            <AnimatePresence mode="wait">
                ...
                >
                    ...
                    <div className={`absolute inset-0 opacity-60 ${(company as any)?.logoColor || 'bg-zinc-800'}`} />
                    <img
                        src={(company as any)?.image || company?.cover_image_url}
                        alt={company?.name}
                    ...
                    <div className="relative z-10 space-y-4">
                        ...
                        <div className="bg-black/20 backdrop-blur-sm p-4 rounded-2xl border border-white/10">
                            ...
                            <p className="text-zinc-100 text-[11px] leading-relaxed italic line-clamp-2">
                                「{(company as any)?.rjpNegatives || '情報が登録されていません'}」
                            </p>
                        </div>
